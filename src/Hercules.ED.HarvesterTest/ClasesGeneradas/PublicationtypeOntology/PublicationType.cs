...
namespace PublicationtypeOntology
{
	public class PublicationType : GnossOCBase
	{
		...
		public virtual string RdfType { get { return "http://w3id.org/roh/PublicationType"; } }
		public virtual string RdfsLabel { get { return "http://w3id.org/roh/PublicationType"; } }
		...
		public virtual SecondaryResource ToGnossApiResource(ResourceApi resourceAPI,string identificador)
		{
			...
			GetProperties();
			SecondaryOntology ontology = new SecondaryOntology(resourceAPI.GraphsUrl, resourceAPI.OntologyUrl, "http://w3id.org/roh/PublicationType", "http://w3id.org/roh/PublicationType", prefList, propList,identificador,listSecondaryEntity, null);
		...
		public override List<string> ToOntologyGnossTriples(ResourceApi resourceAPI)
		{
			...
			AgregarTripleALista($"{resourceAPI.GraphsUrl}items/PublicationType_{ResourceID}_{ArticleID}", "http://www.w3.org/1999/02/22-rdf-syntax-ns#type", $"<http://w3id.org/roh/PublicationType>", list, " . ");
			AgregarTripleALista($"{resourceAPI.GraphsUrl}items/PublicationType_{ResourceID}_{ArticleID}", "http://www.w3.org/2000/01/rdf-schema#label", $"\"http://w3id.org/roh/PublicationType\"", list, " . ");
			AgregarTripleALista($"{resourceAPI.GraphsUrl}{ResourceID}", "http://gnoss/hasEntidad", $"<{resourceAPI.GraphsUrl}items/PublicationType_{ResourceID}_{ArticleID}>", list, " . ");
				if(this.Dc_title != null)
				{
							foreach (LanguageEnum idioma in this.Dc_title.Keys)
							{
								AgregarTripleALista($"{resourceAPI.GraphsUrl}items/PublicationType_{ResourceID}_{ArticleID}", "http://purl.org/dc/elements/1.1/title",  $"\"{GenerarTextoSinSaltoDeLinea(this.Dc_title[idioma])}\"", list,  $"{idioma} . ");
							}
				}
				if(this.Dc_identifier != null)
				{
					AgregarTripleALista($"{resourceAPI.GraphsUrl}items/PublicationType_{ResourceID}_{ArticleID}",  "http://purl.org/dc/elements/1.1/identifier", $"\"{GenerarTextoSinSaltoDeLinea(this.Dc_identifier)}\"", list, " . ");
				}
			return list;
		}
		...
		{
			return $"{resourceAPI.GraphsUrl}items/PublicationtypeOntology_{ResourceID}_{ArticleID}";
		}
