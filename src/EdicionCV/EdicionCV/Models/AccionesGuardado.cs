    ...
    public class AccionesGuardado
    {
        ...
        public JsonResult ChangePrivacityItem(string pIdSection, string pRdfTypeTab, string pEntity, bool pIsPublic)
        {
            ...
            if (string.IsNullOrEmpty(valorActual))
            {
                ...
                TriplesToInclude tr = new TriplesToInclude(valor, propiedad);
                ...
                respuesta.ok = dicCorrecto[mResourceApi.GetShortGuid(pEntity)];
                return respuesta;
            }
            else
            {
                //Modificamos la propiedad
                Dictionary<Guid, List<TriplesToModify>> dicModify = new Dictionary<Guid, List<Gnoss.ApiWrapper.Model.TriplesToModify>>();
                ...
                List<TriplesToModify> listaModify = new List<TriplesToModify>();
                listaModify.Add(tripe);
                dicModify.Add(mResourceApi.GetShortGuid(pEntity), listaModify);
        ...
        public JsonResult ActualizarEntidad(Entity pEntity, string pCvID, string pSectionID, string pRdfTypeTab)
        {
            if (!string.IsNullOrEmpty(pSectionID) && !string.IsNullOrEmpty(pRdfTypeTab))
            {
                //Item de CV
                //TODO X
                TabTemplate template = UtilityCV.TabTemplates.First(x => x.rdftype == pRdfTypeTab);
                Section templateSection = template.sections.First(x => x.property == pSectionID);
                mResourceApi.ChangeOntoly(templateSection.presentation.listItemEdit.graph);
                pEntity.ontology = templateSection.presentation.listItemEdit.graph;
                if (string.IsNullOrEmpty(pEntity.id))
                {
                    //Creamos
                    pEntity.propTitle = templateSection.presentation.listItemEdit.proptitle;
                    pEntity.propDescription = templateSection.presentation.listItemEdit.propdescription;
                    ...
                    if (resource.Uploaded && !string.IsNullOrEmpty(templateSection.property))
                    {
                        ...
                        //Entidad
                        string predicadoEntidad = template.property + "|" + templateSection.property + "|" + templateSection.presentation.listItemEdit.property;
                        ...
                        triplesToInclude.Add(mResourceApi.GetShortGuid(pCvID), listaTriples);
                ...
                else
                {
                    ...
                    loadedEntity.propTitle = templateSection.presentation.listItemEdit.proptitle;
                    loadedEntity.propDescription = templateSection.presentation.listItemEdit.propdescription;
            ...
            {
                //Entidad
                ListItemEdit itemEdit = UtilityCV.EntityTemplates.First(x => x.rdftype == pEntity.rdfType);
        ...
        //TODO
        private bool ProcesCompossedProperties(ListItemEdit pItemEdit, Entity pLoadedEntity)
        {
            bool changes = false;
            foreach (ListItemEditSection section in pItemEdit.sections)
            {
                foreach (ListItemEditSectionRow sectionRow in section.rows)
                {
        ...
        private bool ProcesCompossedPropertieRow(ListItemEditSectionRow pListItemEditSectionRow, Entity pLoadedEntity, string pAcumulado)
        {
            bool changes = false;
            foreach (ListItemEditSectionRowProperty sectionRowProperty in pListItemEditSectionRow.properties)
            {
                if (sectionRowProperty.auxEntityData != null)
                {
                    foreach (ListItemEditSectionRow sectionRow in sectionRowProperty.auxEntityData.rows)
                    {
                ...
                if (!string.IsNullOrEmpty(sectionRowProperty.compossed))
                {
                    ...
                    if (!string.IsNullOrEmpty(value))
                    {
                        ...
                        prop.prop = sectionRowProperty.property;
                        prop.values = new List<string>() { value};
        ...
        private ComplexOntologyResource ToGnossApiResource(Entity pEntity)
        {
            ...
            foreach (Entity.Property property in pEntity.properties.OrderBy(x => x.prop.Split(new string[] { "@@@" }, StringSplitOptions.RemoveEmptyEntries).Length))
            {
                if (property.values.Count > 0)
                {
                    ...
                    foreach (string value in property.values)
                    {
                        string[] valueArray = value.Split(new string[] { "@@@" }, StringSplitOptions.None);
                        ...
                        if (propArray.Count() == 1)
                        {
                            ...
                            entidadPrincipal.props[propPrefix].Add(value);
                        }
                        else
                        {
                            ...
                            if (entidadActual == null)
                            {
                                ...
                                entidadActual.props = new Dictionary<string, List<string>>();
                                entidadActual.ents = new Dictionary<string, List<EntityRdf>>();
                                entidadActual.rdfType = rdfType;
                                entidadActual.id = id;
            ...
            resource.Ontology = ProcesarEntidadPrincipal(entidadPrincipal);
        ...
        private Entity GetLoadedEntity(string pId, string pGraph)
        {
            ...
            if (listResult.Count > 0 && listResult.ContainsKey(pId))
            {
                ...
                entity.id = pId;
                entity.ontology = pGraph;
                entity.rdfType = listResult[pId].First(x => x["p"].value == "http://www.w3.org/1999/02/22-rdf-syntax-ns#type")["o"].value;
                ...
                if (s == pId && p != "http://www.w3.org/2000/01/rdf-schema#label" && p != "http://www.w3.org/1999/02/22-rdf-syntax-ns#type")
                {
                    ...
                    else
                    {
                        ...
                        if (property == null)
                        {
                            ...
                            property.prop = pPropAcumuladoAux;
