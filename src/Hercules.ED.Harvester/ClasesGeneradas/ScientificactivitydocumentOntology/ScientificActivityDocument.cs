...
namespace ScientificactivitydocumentOntology
{
	public class ScientificActivityDocument : GnossOCBase
	{
		...
		internal override void GetProperties()
		{
			...
			if(this.Bibo_abstract != null)
			{
				...
				{
					propList.Add(new StringOntologyProperty("bibo:abstract", this.Bibo_abstract[idioma], idioma.ToString()));
				}
			}
			else
			{
				throw new GnossAPIException($"La propiedad bibo:abstract debe tener al menos un valor en el recurso: {resourceID}");
			}
		}
		...
		public virtual SecondaryResource ToGnossApiResource(ResourceApi resourceAPI,string identificador)
		{
			...
			GetProperties();
			SecondaryOntology ontology = new SecondaryOntology(resourceAPI.GraphsUrl, resourceAPI.OntologyUrl, "http://w3id.org/roh/ScientificActivityDocument", "http://w3id.org/roh/ScientificActivityDocument", prefList, propList,identificador,listSecondaryEntity, null);
		...
		public override List<string> ToOntologyGnossTriples(ResourceApi resourceAPI)
		{
			...
			AgregarTripleALista($"{resourceAPI.GraphsUrl}items/ScientificActivityDocument_{ResourceID}_{ArticleID}", "http://www.w3.org/1999/02/22-rdf-syntax-ns#type", $"<http://w3id.org/roh/ScientificActivityDocument>", list, " . ");
			AgregarTripleALista($"{resourceAPI.GraphsUrl}items/ScientificActivityDocument_{ResourceID}_{ArticleID}", "http://www.w3.org/2000/01/rdf-schema#label", $"\"http://w3id.org/roh/ScientificActivityDocument\"", list, " . ");
			AgregarTripleALista($"{resourceAPI.GraphsUrl}{ResourceID}", "http://gnoss/hasEntidad", $"<{resourceAPI.GraphsUrl}items/ScientificActivityDocument_{ResourceID}_{ArticleID}>", list, " . ");
				if(this.Dc_title != null)
				{
							foreach (LanguageEnum idioma in this.Dc_title.Keys)
							{
								AgregarTripleALista($"{resourceAPI.GraphsUrl}items/ScientificActivityDocument_{ResourceID}_{ArticleID}", "http://purl.org/dc/elements/1.1/title",  $"\"{GenerarTextoSinSaltoDeLinea(this.Dc_title[idioma])}\"", list,  $"{idioma} . ");
							}
				}
				if(this.Dc_identifier != null)
				{
					AgregarTripleALista($"{resourceAPI.GraphsUrl}items/ScientificActivityDocument_{ResourceID}_{ArticleID}",  "http://purl.org/dc/elements/1.1/identifier", $"\"{GenerarTextoSinSaltoDeLinea(this.Dc_identifier)}\"", list, " . ");
				}
				if(this.Bibo_abstract != null)
				{
							foreach (LanguageEnum idioma in this.Bibo_abstract.Keys)
							{
								AgregarTripleALista($"{resourceAPI.GraphsUrl}items/ScientificActivityDocument_{ResourceID}_{ArticleID}", "http://purl.org/ontology/bibo/abstract",  $"\"{GenerarTextoSinSaltoDeLinea(this.Bibo_abstract[idioma])}\"", list,  $"{idioma} . ");
							}
				}
			return list;
		}
		...
		{
			return $"{resourceAPI.GraphsUrl}items/ScientificactivitydocumentOntology_{ResourceID}_{ArticleID}";
		}
