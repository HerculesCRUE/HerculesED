...
namespace DegreetypeOntology
{
	[ExcludeFromCodeCoverage]
	public class DegreeType : GnossOCBase
	{

		public DegreeType() : base() { } 

		public DegreeType(SemanticEntityModel pSemCmsModel, LanguageEnum idiomaUsuario) : base()
		{
		...
		public virtual SecondaryResource ToGnossApiResource(ResourceApi resourceAPI,string identificador)
		{
			...
			GetProperties();
			SecondaryOntology ontology = new SecondaryOntology(resourceAPI.GraphsUrl, resourceAPI.OntologyUrl, "http://w3id.org/roh/DegreeType", "http://w3id.org/roh/DegreeType", prefList, propList,identificador,listSecondaryEntity, null);
		...
		public override List<string> ToOntologyGnossTriples(ResourceApi resourceAPI)
		{
			...
			AgregarTripleALista($"{resourceAPI.GraphsUrl}items/DegreeType_{ResourceID}_{ArticleID}", "http://www.w3.org/1999/02/22-rdf-syntax-ns#type", $"<http://w3id.org/roh/DegreeType>", list, " . ");
			AgregarTripleALista($"{resourceAPI.GraphsUrl}items/DegreeType_{ResourceID}_{ArticleID}", "http://www.w3.org/2000/01/rdf-schema#label", $"\"http://w3id.org/roh/DegreeType\"", list, " . ");
			AgregarTripleALista($"{resourceAPI.GraphsUrl}{ResourceID}", "http://gnoss/hasEntidad", $"<{resourceAPI.GraphsUrl}items/DegreeType_{ResourceID}_{ArticleID}>", list, " . ");
				if(this.Dc_title != null)
				{
							foreach (LanguageEnum idioma in this.Dc_title.Keys)
							{
								AgregarTripleALista($"{resourceAPI.GraphsUrl}items/DegreeType_{ResourceID}_{ArticleID}", "http://purl.org/dc/elements/1.1/title",  $"\"{GenerarTextoSinSaltoDeLinea(this.Dc_title[idioma])}\"", list,  $"{idioma} . ");
							}
				}
