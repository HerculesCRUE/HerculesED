...
namespace ScientificexperienceprojectOntology
{
	public class ScientificExperienceProject : GnossOCBase
	{
		...
		public virtual SecondaryResource ToGnossApiResource(ResourceApi resourceAPI,string identificador)
		{
			...
			GetProperties();
			SecondaryOntology ontology = new SecondaryOntology(resourceAPI.GraphsUrl, resourceAPI.OntologyUrl, "http://w3id.org/roh/ScientificExperienceProject", "http://w3id.org/roh/ScientificExperienceProject", prefList, propList,identificador,listSecondaryEntity, null);
		...
		public override List<string> ToOntologyGnossTriples(ResourceApi resourceAPI)
		{
			...
			AgregarTripleALista($"{resourceAPI.GraphsUrl}items/ScientificExperienceProject_{ResourceID}_{ArticleID}", "http://www.w3.org/1999/02/22-rdf-syntax-ns#type", $"<http://w3id.org/roh/ScientificExperienceProject>", list, " . ");
			AgregarTripleALista($"{resourceAPI.GraphsUrl}items/ScientificExperienceProject_{ResourceID}_{ArticleID}", "http://www.w3.org/2000/01/rdf-schema#label", $"\"http://w3id.org/roh/ScientificExperienceProject\"", list, " . ");
			AgregarTripleALista($"{resourceAPI.GraphsUrl}{ResourceID}", "http://gnoss/hasEntidad", $"<{resourceAPI.GraphsUrl}items/ScientificExperienceProject_{ResourceID}_{ArticleID}>", list, " . ");
				if(this.Dc_title != null)
				{
							foreach (LanguageEnum idioma in this.Dc_title.Keys)
							{
				...
				{
					AgregarTripleALista($"{resourceAPI.GraphsUrl}items/ScientificExperienceProject_{ResourceID}_{ArticleID}",  "http://purl.org/dc/elements/1.1/identifier", $"\"{GenerarTextoSinSaltoDeLinea(this.Dc_identifier)}\"", list, " . ");
				}
				if(this.Bibo_abstract != null)
				{
							foreach (LanguageEnum idioma in this.Bibo_abstract.Keys)
		...
		{
			return $"{resourceAPI.GraphsUrl}items/ScientificexperienceprojectOntology_{ResourceID}_{ArticleID}";
		}
