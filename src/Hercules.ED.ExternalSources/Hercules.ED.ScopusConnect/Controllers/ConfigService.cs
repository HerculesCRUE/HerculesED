...
namespace ScopusAPI.Controllers
{    
    public class ConfigService
    {
        ...
        private string urlScopus_base{get;set;}
                private string urlScopus_key{get;set;}
        ...
        /// </summary>
        /// <returns>URI del API de Scopus.</returns>
        public string GetUrlScopus_base()
        {
            if (string.IsNullOrEmpty(urlScopus_base))
            {
                string connectionString = string.Empty;
                IDictionary environmentVariables = Environment.GetEnvironmentVariables();
                if (environmentVariables.Contains("urlScopus_base"))
                {
                    connectionString = environmentVariables["urlScopus_base"] as string;
                }
                else
                {
                    connectionString = configuracion["urlScopus_base"];
                }

                urlScopus_base = connectionString;
            }

            return urlScopus_base;
        }
        ...
        /// </summary>
        /// <returns>URI del API de Scopus.</returns>
        public string GetUrlScopus_key()
        {
            if (string.IsNullOrEmpty(urlScopus_key))
            {
                string connectionString = string.Empty;
                IDictionary environmentVariables = Environment.GetEnvironmentVariables();
                if (environmentVariables.Contains("urlScopus_key"))
                {
                    connectionString = environmentVariables["urlScopus_key"] as string;
                }
                else
                {
                    connectionString = configuracion["urlScopus_key"];
                }

                urlScopus_key = connectionString;
            }

            return urlScopus_key;
        }
