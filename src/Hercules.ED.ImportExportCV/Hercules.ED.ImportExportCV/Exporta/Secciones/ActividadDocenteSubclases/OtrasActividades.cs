...
namespace ImportadorWebCV.Exporta.Secciones.ActividadDocenteSubclases
{
    public class OtrasActividades : SeccionBase
    {
        private readonly List<string> propiedadesItem = new()
        {
            ...
            "http://w3id.org/roh/otherActivities",
        ...
        public void ExportaOtrasActividades(Dictionary<string, List<Dictionary<string, Data>>> MultilangProp, [Optional] List<string> listaId)
        {
            ...
            List<Tuple<string, string>> listadoIdentificadoresOtrAct = UtilityExportar.GetListadoEntidades(mResourceApi, propiedadesItem, mCvID);
            if (!UtilityExportar.Iniciar(mResourceApi, propiedadesItem, mCvID, listadoIdentificadoresOtrAct, listaId))
            {
                return;
            }

            Dictionary<string, Entity> listaEntidadesOtrAct = GetListLoadedEntity(listadoIdentificadoresOtrAct, graph, MultilangProp);
            foreach (KeyValuePair<string, Entity> keyValue in listaEntidadesOtrAct)
            {
