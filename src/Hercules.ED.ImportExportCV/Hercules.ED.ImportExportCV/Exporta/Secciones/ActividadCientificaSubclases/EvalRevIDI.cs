...
namespace ImportadorWebCV.Exporta.Secciones.ActividadCientificaSubclases
{
    public class EvalRevIDI : SeccionBase
    {
        private readonly List<string> propiedadesItem = new()
        {
            ...
            "http://w3id.org/roh/researchEvaluations",
        ...
        public void ExportaEvalRevIDI(Dictionary<string, List<Dictionary<string, Data>>> MultilangProp, [Optional] List<string> listaId)
        {
            ...
            List<Tuple<string, string>> listadoIdentificadoresEvalRevIdi = UtilityExportar.GetListadoEntidades(mResourceApi, propiedadesItem, mCvID);
            if (!UtilityExportar.Iniciar(mResourceApi, propiedadesItem, mCvID, listadoIdentificadoresEvalRevIdi, listaId))
            {
                return;
            }

            Dictionary<string, Entity> listaEntidadesEvalRevIdi = GetListLoadedEntity(listadoIdentificadoresEvalRevIdi, graph, MultilangProp);
            foreach (KeyValuePair<string, Entity> keyValue in listaEntidadesEvalRevIdi)
            {
