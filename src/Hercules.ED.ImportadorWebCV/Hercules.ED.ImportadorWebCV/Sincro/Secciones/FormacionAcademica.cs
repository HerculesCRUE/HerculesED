...
namespace ImportadorWebCV.Sincro.Secciones
{
    class FormacionAcademica : SeccionBase
    {
        ...
        /// </summary>
        public List<SubseccionItem> SincroFormacionSanitaria([Optional] bool preimportar)
        {
            List<string> propiedadesItem = new List<string>() { "http://w3id.org/roh/qualifications", "http://w3id.org/roh/XXXXXXXXXXXXX", "XXXXXXXXXXXXXXXX" };
            string graph = "XXXXXXXXXXXXXXX";
            string propTitle = "XXXXXXXXXXXXXXXXXX";
            string rdfType = "XXXXXXXXXXXXXXXXX";
            string rdfTypePrefix = "RelatedXXXXXXXXXXXXXX";
            ...
            foreach (Entity entityXML in listadoAux)
            {
                FormacionSanitaria formacionSanitaria = new FormacionSanitaria();
                formacionSanitaria.descripcion = entityXML.properties.FirstOrDefault(x => x.prop == Variables.FormacionAcademica.formacionSanitariaEspeTituloEspe)?.values.FirstOrDefault();
                formacionSanitaria.fecha = entityXML.properties.FirstOrDefault(x => x.prop == Variables.FormacionAcademica.formacionSanitariaEspeFechaInicio)?.values.FirstOrDefault();
                formacionSanitaria.ID = Guid.NewGuid().ToString();
                entidadesXML.Add(formacionSanitaria.ID, formacionSanitaria);
            }

            //2º Obtenemos las entidades de la BBDD
            Dictionary<string, DisambiguableEntity> entidadesBBDD = FormacionSanitaria.GetBBDD(mResourceApi, mCvID, graph, propiedadesItem);
        ...
        /// </summary>
        public List<SubseccionItem> SincroFormacionSanitariaIMasD([Optional] bool preimportar)
        {
            List<string> propiedadesItem = new List<string>() { "http://w3id.org/roh/qualifications", "http://w3id.org/roh/XXXXXXXXXXXXX", "XXXXXXXXXXXXXXXX" };
            string graph = "XXXXXXXXXXXXXXX";
            string propTitle = "XXXXXXXXXXXXXXXXXX";
            ...
            foreach (Entity entityXML in listadoAux)
            {
                FormacionSanitariaImasD formacionSanitariaImasD = new FormacionSanitariaImasD();
                formacionSanitariaImasD.descripcion = entityXML.properties.FirstOrDefault(x => x.prop == Variables.FormacionAcademica.formacionSanitariaIDNombre)?.values.FirstOrDefault();
                formacionSanitariaImasD.fecha = entityXML.properties.FirstOrDefault(x => x.prop == Variables.FormacionAcademica.formacionSanitariaIDFechaInicio)?.values.FirstOrDefault();
                formacionSanitariaImasD.ID = Guid.NewGuid().ToString();
                entidadesXML.Add(formacionSanitariaImasD.ID, formacionSanitariaImasD);
            }

            //2º Obtenemos las entidades de la BBDD
            Dictionary<string, DisambiguableEntity> entidadesBBDD = FormacionSanitariaImasD.GetBBDD(mResourceApi, mCvID, graph, propiedadesItem);
            ...
            if (listadoFormacionSanitariaIMasD.Count > 0)
            {
                foreach (CvnItemBean item in listadoFormacionSanitariaIMasD)
                {
                    ...
                    if (!string.IsNullOrEmpty(item.GetStringPorIDCampo("020.030.000.010")))//TODO -check
                    {
                        ...
                        FormacionSanitariaEntidadRealizacion(item, entidadAux);
                        FormacionSanitariaEntidadTitulacion(item, entidadAux);
        ...
        private void FormacionSanitariaEntidadRealizacion(CvnItemBean item, Entity entidadAux)
        {
            /*             
             ...
             */
            //Añado la referencia si existe Entidad de Titulacion
            UtilitySecciones.AniadirEntidad(mResourceApi, item.GetNameEntityBeanPorIDCampo("020.030.000.070"),
                Variables.FormacionAcademica.formacionSanitariaEspeEntidadRealizacionNombre,
                Variables.FormacionAcademica.formacionSanitariaEspeEntidadRealizacion, entidadAux);

            //Añado otros, o el ID de una preseleccion
            string valorTipo = !string.IsNullOrEmpty(item.GetStringPorIDCampo("020.030.000.100")) ? mResourceApi.GraphsUrl + "items/organizationtype_OTHERS" : item.GetOrganizacionPorIDCampo("020.030.000.090");

            entidadAux.properties.AddRange(UtilitySecciones.AddProperty(
                new Property(Variables.FormacionAcademica.formacionSanitariaEspeTipoEntidadRealizacion, valorTipo),
                new Property(Variables.FormacionAcademica.formacionSanitariaEspeTipoEntidadRealizacionOtros, item.GetStringPorIDCampo("020.030.000.100"))
            ));
        }

        private void FormacionSanitariaEntidadTitulacion(CvnItemBean item, Entity entidadAux)
        {
            //Añado la referencia si existe Entidad de Titulacion
            UtilitySecciones.AniadirEntidad(mResourceApi, item.GetNameEntityBeanPorIDCampo("020.030.000.110"),
                Variables.FormacionAcademica.formacionSanitariaEspeEntidadTitulacionNombre,
                Variables.FormacionAcademica.formacionSanitariaEspeEntidadTitulacion, entidadAux);

            //Añado otros, o el ID de una preseleccion
            string valorTipo = !string.IsNullOrEmpty(item.GetStringPorIDCampo("020.030.000.140")) ? mResourceApi.GraphsUrl + "items/organizationtype_OTHERS" : item.GetOrganizacionPorIDCampo("020.030.000.130");

            entidadAux.properties.AddRange(UtilitySecciones.AddProperty(
                new Property(Variables.FormacionAcademica.formacionSanitariaEspeTipoEntidadTitulacion, valorTipo),
                new Property(Variables.FormacionAcademica.formacionSanitariaEspeTipoEntidadTitulacionOtros, item.GetStringPorIDCampo("020.030.000.140"))
            ));
        }
        #endregion

        #region FormacionSanitariaIMasD
        /// <summary>
        /// 020.040.000.000
        /// </summary>
        /// <param name="listadoDatos"></param>
            ...
            if (listadoFormacionSanitariaIMasD.Count > 0)
            {
                foreach (CvnItemBean item in listadoFormacionSanitariaIMasD)
                {
                    ...
                    if (!string.IsNullOrEmpty(item.GetStringPorIDCampo("020.040.000.010")))//TODO -check
                    {
                        ...
                        FormacionSanitariaIDEntidadTitulacion(item, entidadAux);
                        FormacionSanitariaIDEntidadRealizacion(item, entidadAux);
        ...
        private void FormacionSanitariaIDEntidadTitulacion(CvnItemBean item, Entity entidadAux)
        {
            //Añado la referencia si existe Entidad de Titulacion
            UtilitySecciones.AniadirEntidad(mResourceApi, item.GetNameEntityBeanPorIDCampo("020.040.000.100"),
                Variables.FormacionAcademica.formacionSanitariaIDEntidadTitulacionNombre,
                Variables.FormacionAcademica.formacionSanitariaIDEntidadTitulacion, entidadAux);

            //Añado otros, o el ID de una preseleccion
            string valorTipo = !string.IsNullOrEmpty(item.GetStringPorIDCampo("020.040.000.130")) ? mResourceApi.GraphsUrl + "items/organizationtype_OTHERS" : item.GetOrganizacionPorIDCampo("020.040.000.120");

            entidadAux.properties.AddRange(UtilitySecciones.AddProperty(
                new Property(Variables.FormacionAcademica.formacionSanitariaIDTipoEntidadTitulacion, valorTipo),
                new Property(Variables.FormacionAcademica.formacionSanitariaIDTipoEntidadTitulacionOtros, item.GetStringPorIDCampo("020.040.000.130"))
            ));
        }

        private void FormacionSanitariaIDEntidadRealizacion(CvnItemBean item, Entity entidadAux)
        {
            //Añado la referencia si existe Entidad de Realización
            UtilitySecciones.AniadirEntidad(mResourceApi, item.GetNameEntityBeanPorIDCampo("020.040.000.060"),
                Variables.FormacionAcademica.formacionSanitariaIDEntidadRealizacionNombre,
                Variables.FormacionAcademica.formacionSanitariaIDEntidadRealizacion, entidadAux);

            //Añado otros, o el ID de una preseleccion
            string valorTipo = !string.IsNullOrEmpty(item.GetStringPorIDCampo("020.040.000.090")) ? mResourceApi.GraphsUrl + "items/organizationtype_OTHERS" : item.GetOrganizacionPorIDCampo("020.040.000.080");

            entidadAux.properties.AddRange(UtilitySecciones.AddProperty(
                new Property(Variables.FormacionAcademica.formacionSanitariaIDTipoEntidadRealizacion, valorTipo),
                new Property(Variables.FormacionAcademica.formacionSanitariaIDTipoEntidadRealizacionOtros, item.GetStringPorIDCampo("020.040.000.090"))
            ));
        }
        #endregion
        ...
        private List<Entity> GetConocimientoIdiomas(List<CvnItemBean> listadoDatos)
        {
            ...
            if (listadoConocimientoIdiomas.Count > 0)
            {
                foreach (CvnItemBean item in listadoConocimientoIdiomas)
                {
                    ...
                    //Si el TitleBean es nulo o no tiene identificador, no haog nada. 
