...
```
curl -X POST -H 'Content-Type: application/json' -i 'http://herculesapi.elhuyar.eus/thematic' --data '{"text":"Command line HTML calendar generator. Python 3 based HTML calendar generator. Generates either a single calendar month or a set of 12 calendar months for a given year. Adjust for leap year and system file separator differences.","rotype":"sourceForge"}'
```
