...
{
    class FormacionSanitaria : DisambiguableEntity
    {
        ...
        public static Dictionary<string, DisambiguableEntity> GetBBDD(ResourceApi pResourceApi, string pCVID, string graph, List<string> propiedadesItem)
        {
            ...
            foreach (List<string> lista in listaListas)
            {
                ...
                string where = $@"where {{
                                        ?item <{Variables.FormacionAcademica.formacionSanitariaEspeTituloEspe}> ?itemTitle . 
                                        OPTIONAL{{?item <{Variables.FormacionAcademica.formacionSanitariaEspeFechaInicio}> ?itemDate }}.
                ...
                foreach (Dictionary<string, Data> fila in resultData.results.bindings)
                {
                    ...
                    formacionSanitaria.ID = fila["item"].value;
                    formacionSanitaria.descripcion = fila["itemTitle"].value;
                    formacionSanitaria.fecha = "";
                    if (fila.ContainsKey("itemDate"))
                    {
                        formacionSanitaria.fecha = fila["itemDate"].value;
                    }
                    resultados.Add(pResourceApi.GetShortGuid(fila["item"].value).ToString(), formacionSanitaria);
                }
            }
