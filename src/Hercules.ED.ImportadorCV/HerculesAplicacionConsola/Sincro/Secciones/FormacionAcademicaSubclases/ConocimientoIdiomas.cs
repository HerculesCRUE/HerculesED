...
namespace HerculesAplicacionConsola.Sincro.Secciones.FormacionAcademicaSubclases
{
    class ConocimientoIdiomas : DisambiguableEntity
    {
        ...
        private static DisambiguationDataConfig configIdioma = new DisambiguationDataConfig()
        {
        ...
        public static Dictionary<string, DisambiguableEntity> GetBBDD(ResourceApi pResourceApi, string pCVID, string graph, List<string> propiedadesItem)
        {
            ...
            foreach (List<string> lista in listaListas)
            {
                ...
                foreach (Dictionary<string, Data> fila in resultData.results.bindings)
                {
                    ...
                    conocimientoIdiomas.ID = fila["item"].value;
                    conocimientoIdiomas.idioma = fila["itemTitle"].value;
