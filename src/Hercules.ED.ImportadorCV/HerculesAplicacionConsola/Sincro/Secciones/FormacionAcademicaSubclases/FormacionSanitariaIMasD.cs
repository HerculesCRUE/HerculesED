...
{
    class FormacionSanitariaImasD : DisambiguableEntity
    {
        ...
        public static Dictionary<string, DisambiguableEntity> GetBBDD(ResourceApi pResourceApi, string pCVID, string graph, List<string> propiedadesItem)
        {
            ...
            foreach (List<string> lista in listaListas)
            {
                ...
                string where = $@"where {{
                                        ?item <{Variables.FormacionAcademica.formacionSanitariaIDNombre}> ?itemTitle . 
                                        OPTIONAL{{?item <{Variables.FormacionAcademica.formacionSanitariaIDFechaInicio}> ?itemDate }}.
                ...
                {
                    FormacionSanitariaImasD formacionSanitariaID = new FormacionSanitariaImasD();
                    formacionSanitariaID.ID = fila["item"].value;
                    formacionSanitariaID.descripcion = fila["itemTitle"].value;
                    formacionSanitariaID.fecha = "";
                    if (fila.ContainsKey("itemDate"))
                    {
                        formacionSanitariaID.fecha = fila["itemDate"].value;
                    }
                    resultados.Add(pResourceApi.GetShortGuid(fila["item"].value).ToString(), formacionSanitariaID);
                }
            }
