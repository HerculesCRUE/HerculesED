    ...
    class AcreditacionesReconocimientos : DisambiguableEntity
    {
        ...
        public static Dictionary<string, DisambiguableEntity> GetBBDD(ResourceApi pResourceApi, string pCVID, string graph, List<string> propiedadesItem)
        {
            ...
            foreach (List<string> lista in listaListas)
            {
                ...
                string where = $@"where {{
                                        ?item <{Variables.ActividadCientificaTecnologica.acreditacionesNombre}> ?itemTitle . 
                ...
                foreach (Dictionary<string, Data> fila in resultData.results.bindings)
                {
                    ...
                    acreditacionesReconocimientos.ID = fila["item"].value;
                    acreditacionesReconocimientos.descripcion = fila["itemTitle"].value;
                    acreditacionesReconocimientos.fecha = "";
                    if (fila.ContainsKey("itemDate"))
                    {
                        acreditacionesReconocimientos.fecha = fila["itemDate"].value;
                    }
