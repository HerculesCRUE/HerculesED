...
function administrarGrupos(pContenedor, pIdPagina, pIdGrafica, titulo, tipoGrafica, groupID) {
    var isZoom = $(pContenedor).parents("#modal-ampliar-mapa").length > 0;
    var grupoGrafica = groupID || "-" + pIdPagina + "_" + pIdGrafica;
    $(pContenedor).data("grupo", grupoGrafica);
    if (grupos[grupoGrafica] == undefined || isZoom) {
        var first = true;
        //$(pContenedor).addClass("show");
        if (!$(pContenedor).parent().hasClass("wrap") && !$(pContenedor).parent().hasClass("modal-body")) {
            $(pContenedor).parent().append(`
            <div class="wrap" data-grupo="${grupoGrafica}"/>
        `);
            $(pContenedor).appendTo($(pContenedor).parent().find(".wrap"));
        }
        if (!isZoom) {
            grupos[grupoGrafica] = {
                graficas: {},
                parent: $(pContenedor).parent()
            }
        }
    }

    grupos[grupoGrafica].graficas[pIdGrafica + "_" + pIdPagina] = {
        idGrafica: pIdGrafica,
        idPagina: pIdPagina,
        titulo: titulo,
        ...
        callbacks: {
            ...
            download: addDownloadButton.bind(null, pContenedor),
        }
    };

    if ($(pContenedor).data("grupo") && grupos[grupoGrafica] != undefined && !first) {

        if (!$(pContenedor).hasClass("show")) {
            $(pContenedor).addClass("hide");
            $(pContenedor).css("opacity", "0");
            $(pContenedor).css("position", "absolute");
            $(pContenedor).css("top", "-9999px");
            $(pContenedor).css("left", "-9999px");
            $(pContenedor).css("z-index", "-1");
        }
        $(pContenedor).appendTo(grupos[grupoGrafica].parent);

        if (!$(grupos[grupoGrafica].parent).find(".acciones-mapa").hasClass("showAcciones")) {
            $(grupos[grupoGrafica].parent).find(".acciones-mapa").addClass("showAcciones");
            $(grupos[grupoGrafica].parent).find(".acciones-mapa").prepend(`
            <div class="toggleGraficas">
                <a href="javascript: void(0);" id="dropdownMasOpciones" data-toggle="dropdown">
                    <span class="material-icons">sync_alt</span>
                </a>
            ...
            $(grupos[grupoGrafica].parent).find(".acciones-mapa").find("ul").empty();
            Object.keys(grupos[grupoGrafica].graficas).forEach(function (key, index) {
                ///console.log(grupos[grupoGrafica].graficas[key]);
                var grafica = grupos[grupoGrafica].graficas[key];
                $(grupos[grupoGrafica].parent).find(".acciones-mapa").find("ul").append(`
                <a order="${index}" value="grafica_${grafica.idPagina}_${grafica.idGrafica}" class="item-dropdown">
                    <span class="material-icons">${tipo}</span>
                    <span class="texto">${grafica.titulo}</span>
                </a>
            `);
            });
        }
        return grupos[grupoGrafica].graficas[pIdGrafica + "_" + pIdPagina];
    }
    $(pContenedor).attr("orden", Object.keys(grupos[grupoGrafica].graficas) - 1);

}
function pintarContenedoresGraficas(pContenedor, pIdPagina = "", tipografica = "", pIdGrafica = "", groupId = "") {
    var isZoom = $(pContenedor).parents("#modal-ampliar-mapa").length > 0;
    var ctx;
    var grupoGrafica = groupId || "-" + pIdPagina + "_" + pIdGrafica;
    ...
    if (tipografica.includes("nodes")) {
        $(pContenedor).append(`
            ...
            `);
        ctx = $(`<div class="graficoNodos" id="grafica_${pIdPagina}_${pIdGrafica}" style="height: 100%;"></div>`);
        $(pContenedor).append(ctx);
        $(pContenedor).parent().parent().css("overflow", "hidden");
    } else {
        ctx = $(`<canvas id = "grafica_${pIdPagina}_${pIdGrafica}" width = "600" height = "250" ></canvas>`)
    ...
    if (!isZoom) { // todo mover
        grupos[grupoGrafica].graficas[pIdGrafica + "_" + pIdPagina]["ctx"] = ctx;
    }
    return ctx[0];
}
function pintarContenedoresPersonalizados(pContenedor, pPageData, tipoGrafica) {
    ...
    if (tipoGrafica == "nodes") {
        $(pContenedor).append(`
            ...
            `);
        ctx = $(`<div class="graficoNodos" id="${pPageData.idRecurso}" style="height: 500px;"></div>`);
        $(pContenedor).append(ctx);
    } else if (tipoGrafica == "circular") {
        $(pContenedor).css("height", "300px");
        ctx = $(`<canvas id = "${pPageData.idRecurso}"></canvas>`)
        $(pContenedor).append(ctx);
    } else {
        ctx = $(`<canvas width = "600" height = "250" id="${pPageData.idRecurso}"></canvas>`)
        $(pContenedor).append(`
        <div class="chartWrapper" >
...
function getGrafica(pIdPagina, pIdGrafica, pFiltroFacetas, ctx = null, barSize = 50, pIdRecurso = null, pTitulo = null, maxScales = null, pPageData = null) {
    ...
    $.get(url, arg, function (data) {
        ...
        var grupo = $(container).data("grupo");
        ...
        if (tipoGrafica == "nodes") {
            ...
            titulo = data.options.plugins.title.text;
        }
        $(ctx).data("titulo", titulo);
        if (!grupo) {
            grupo = data.groupId;
        }
        ...
        if (ctx) {
            if (ctx.tagName == "DIV") {//$(ctx).parents(".grafica").length == 0) { //igual es mejor comprobar si la id es de
                var graficaData = administrarGrupos(ctx, pIdPagina, pIdGrafica, titulo, tipoGrafica, grupo);
                if (pPageData) {
                    ctx = pintarContenedoresPersonalizados(ctx, pPageData, tipoGrafica, grupo)
                } else {
                    ctx = pintarContenedoresGraficas(ctx, pIdPagina, tipoGrafica, pIdGrafica, grupo);
                    //addZoomButton(tmp);
                }
...
function reDrawChart(myChart, mainAxis, secondaryAxis, canvasSize, horizontal = false) {
    var legend = $(myChart.canvas).parents(".chartAreaWrapper").find(".chartLegend")[0];
    /* TODO - Actualizar el tamaño de las barras dependiendo de los datasets visibles.
    myChart.data.datasets.forEach((dataset, index) => { // esto casi funciona, pero hace cosas raras
        dataset['barThickness'] = 50/(myChart.getVisibleDatasetCount());
    })
...
function addDownloadButton(pContenedor) {
    var accionesMapa = pintarAccionesMapa(pContenedor);
    ...
    */
    ...
    $('a.descargar').off('click.img').on('click.img', function (e) {
        ...
        var canvas = $(this).parents('div.acciones-mapa').parent().find('div.grafica.show canvas') || $(this).parents('div.wrap').find('div.chartAreaWrapper canvas');
        var chart = Chart.getChart(canvas);
        // Obtención del chart usando el elemento canvas de graficas sin scroll y de Chart.js
        if (chart == null) {
    ...
    $('a.csv')
        ...
        .click(function (e) {
            var url = url_servicio_graphicengine + "GetCSVGrafica";
            if (!$('div').hasClass('indicadoresPersonalizados')) {
                var id = $(this).parents('div.acciones-mapa').parent().find("div.grafica.show").find('canvas')[0].id || $(this).parents('div.acciones-mapa').parent().find("div.grafica.show").find("div.graficoNodos")[0].id
                var idGrafica = $(this).parents('div.acciones-mapa').parent().find('div.grafica.show').attr('idgrafica') || $(this).parents('div.acciones-mapa').parent().find('div.grafica.show').data('idgrafica');
                url += "?pIdPagina=" + id.split('_')[1];
                url += "&pIdGrafica=" + idGrafica;
                url += "&pFiltroFacetas=" + encodeURIComponent(ObtenerHash2());
                url += "&pLang=" + lang;
                var urlAux = url_servicio_graphicengine + "GetGrafica"; //"https://localhost:44352/GetGrafica"
                var argAux = {};
                argAux.pIdPagina = id.split('_')[1];
                argAux.pIdGrafica = idGrafica;
                ...
                var filtro = $(this).parents('div.acciones-mapa').parent().find('div.grafica.show').attr('filtro');
                if (filtro != "") {
